...
Lima tab laporan (pendapatan, utilisasi armada, pelanggan teratas, klaim, keuangan) + ekspor CSV/PDF **berfungsi**. Tidak ditemukan SQL injection (semua `selectRaw` statis, satu-satunya binding berparameter). Kelemahan utama: **ekspor tanpa gate** (permission-nya ada tapi tak dipakai) dan **filter tanggal tanpa validasi** (input sampah → 500).

| Kategori | Status |
|---|---|
| 🔴 Tinggi (ekspor tanpa izin / crash input) | **TERBUKA — 2 temuan** (RPT-01, RPT-02) |
| 🟡 Sedang (kebenaran query / inkonsistensi) | **TERBUKA — 3 temuan** (RPT-03, RPT-04, RPT-05) |
| 🟢 Rendah (format / performa) | **TERBUKA — 2 temuan** (RPT-06, RPT-07) |

---

## 2. INVENTARIS RUTE & VIEW

Grup `routes/rental.php:346` `['prefix' => 'report', 'as' => 'report.', 'middleware' => ['can:report_view']]` (inherit `web,auth,two_factor`):

| HTTP | URI | Action | Name | Middleware efektif |
|---|---|---|---|---|
...
### [ ] RPT-01 — Ekspor CSV/PDF tanpa gate `report_export` — 🔴
...
### [ ] RPT-02 — Filter tanggal tanpa validasi → 500 — 🔴
...
### [ ] RPT-03 — GROUP BY alias vs `only_full_group_by` — 🟡
...
### [ ] RPT-04 — Inkonsistensi kartu & metrik — 🟡
...
### [ ] RPT-05 — Tidak ada SQL injection (terverifikasi, tanpa aksi) — 🟡→✅
...
### [ ] RPT-06 — Format CSV/PDF tidak konsisten locale id-ID — 🟢
...
### [ ] RPT-07 — Ekspor & grafik tanpa batas — 🟢
...
|---|---|---|---|---|
| RPT-01 | Gate `can:report_export` di kedua rute ekspor (+ nasib `report_download`) | 🔴 Tinggi | Kecil | [ ] Terbuka |
| RPT-02 | Validasi filter tanggal + batas rentang | 🔴 Tinggi | Kecil | [ ] Terbuka |
| RPT-03 | GROUP BY ekspresi mentah (3 titik) | 🟡 Sedang | Kecil | [ ] Terbuka |
| RPT-04 | Samakan `outstanding` ke `$end` + definisi utilisasi | 🟡 Sedang | Kecil | [ ] Terbuka |
| RPT-05 | Verifikasi no-SQLi (bukti, tanpa aksi) | — | — | [X] Bersih |
| RPT-06 | Format locale id-ID CSV/PDF + bulan Indonesia | 🟢 Rendah | Kecil–Sedang | [ ] Terbuka |
| RPT-07 | Throttle + batas rentang ekspor | 🟢 Rendah | Kecil | [ ] Terbuka |
